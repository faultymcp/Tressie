    ...
    supabase
      ...
      .then(async ({ data }) => {
        if (data && data.length > 0) {
          ...
          // Nothing tagged for this hair type yet: show every pick rather
          // than an empty screen.
          const { data: all } = await supabase.from('products').select('*').limit(40);
          setProducts(all || []);
        }
        setLoading(false);
      });
  ...
  return (
    <View style={st.container}>
      ...
      <View style={st.header}>
        ...
        <Text style={st.subtitle}>Picked for Type {hairType || typeGroup} hair</Text>
      </View>
                  ...
                  <View style={st.prodBottom}>
                    ...
                    {p.rating ? (
                      <View style={st.ratingRow}>
                        ...
                        <Text style={st.ratingValue}>{Number(p.rating).toFixed(1)}</Text>
                        {p.review_count ? (
                          <Text style={st.ratingCount}>
...
const st = StyleSheet.create({
  ...
  // Star rating — shown when the product has a real rating in the database,
  // otherwise the retailer name shows instead. Never a made-up number.
  ratingRow: { flexDirection: 'row', alignItems: 'center', gap: 3, flex: 1 },
  ratingStar: { fontSize: 12, color: Colors.lime },
  ratingValue: { fontFamily: Fonts.bodySemi, fontSize: 12, color: Colors.ink },
  ratingCount: { fontFamily: Fonts.body, fontSize: 11, color: Colors.muted },
